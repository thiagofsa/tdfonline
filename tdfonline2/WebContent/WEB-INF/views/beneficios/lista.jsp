...
<div class="container">
	...
	<div   class="text-center text-uppercase font-weight-bold">
		<h4  class="h4 font-weight-bold">Relação de Benefícios</h4>
	</div>
	...
	<div  class="row areanotify justify-content-end py-1">		
		...
		<table id="Tabela" class="table table-striped table-hover display"  style="width:100%">
			...
			<c:forEach var="beneficio" items="${beneficios}">
			    <tr>
					<td>${beneficio.paciente.nome}</td>
					<td class="text-center">${beneficio.dataviagemida}</td>
					<td class="text-center">${beneficio.dataviagemvolta}</td>
					<td class="text-center">${beneficio.valor}</td>
									
					<td class="text-center">
					  <spring:url value="/beneficios/${beneficio.id}/update" var="updateUrl" />
		...
		<div class="row justify-content-center text-center mt-3 animated fadein" >			
			<a class="btn btn-outline-primary"  href="${pageContext.request.contextPath}/beneficioavulsos/add">
			<i class="fas fa-plus-square mx-3"></i><span class="hidea"> Adicionar Benefício</span></a>
		</div>
